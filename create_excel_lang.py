...
pivot_df = df.pivot_table(
    index=["Language"],
    columns=["Model Name", "Retrieval Type", "Metric"],
    values="Value"
)
print(pivot_df)
...
for model_name in model_names:
    for retrieval_type in retrieval_order:
        for metric in metric_order:
            col = (model_name, retrieval_type, metric)
...
for model_name in model_names:
    ...
    ws.cell(row=1, column=col_start, value=model_name.capitalize()).alignment = Alignment(horizontal="center", vertical="center")
    ...
    start_col += 6  # Move to the next model_name group

# Write model data rows
ws.cell(row=3, column=1, value="Languages")
...
# Save the workbook
file_path = "./excels/recall_language_ordered_i2t_t2i.xlsx"
wb.save(file_path)
